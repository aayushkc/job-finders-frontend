...
export default function EmailActivation() {
    ...
    const uidb = params?.activationDetails[0] || undefined
    const token = params?.activationDetails[1] || undefined
    const accessToken = Cookies.get('accessToken') || undefined

    const [responseStatus, setResponseStatus] = useState(null)
    const [mailStatus, setMailStatus] = useState({isPending:false, isComplete:false})

    const handleResendActivationLink =async () =>{
        setMailStatus((prevState) => {return {...prevState,isPending:true}})
        const res = await fetch(`${APIENDPOINT}/resend-activate-link/${uidb}/`, {
            method: "GET",
            headers: {
        ...
        if(res.status === 503){
            toast.error("Server unable to send mail",{
                position: "bottom-right",
                autoClose: 800,
                theme: "light",
            })
            setMailStatus({isPending:false, isComplete:true})
            return;
        }

        if(res.status === 404){
            toast.error("Requested user mail not found",{
                position: "bottom-right",
                autoClose: 800,
                theme: "light",
            })
    ...
    useEffect(() => {
        const getResponseStatus = async () => {
            try {
                ...
                const response = await fetch(`${APIENDPOINT}/activate/${uidb}/${token}`, {
                    method: "GET",
                    headers: {
                        'Content-Type': 'application/json',
                    },
        ...
        <div class="bg-gradient-to-r from-slate-200 to-gray-200 dark:from-gray-800 dark:to-gray-900 text-black dark:text-white">
            <div class="flex items-center justify-center min-h-screen px-2">
                <div class="text-center">
                    ...
                    <p class="mt-4 mb-8">The page you're looking for doesn't exist.</p>
                    <a href="/"
